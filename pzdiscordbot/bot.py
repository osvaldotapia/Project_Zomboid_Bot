...
def obtener_jugadores():
    """
    Envía 'players' a la consola de PZ y parsea los nombres reales
    de los usuarios desde el log.
    """
    jugadores = []
    ...
    try:
        # Enviar "players" a la consola de PZ
        comando = (
        ...
        if resultado_comando.returncode != 0:
            print("❌ No se pudo enviar el comando players.")
            return jugadores

        # Esperar respuesta en el log
        import time
        time.sleep(PLAYER_QUERY_DELAY)
        ...
        if resultado.returncode != 0:
            print(f"❌ Error leyendo log: {resultado.stderr}")
            return jugadores

        lineas = resultado.stdout.splitlines()

        # Recorremos el log de abajo hacia arriba para buscar la respuesta más reciente
        for i in range(len(lineas) - 1, -1, -1):
            linea = lineas[i]

            # Buscar la cabecera: "Players connected (X):"
            coincidencia = re.search(r"Players connected \((\d+)\):", linea)
            ...
            if coincidencia:
                ...
                # Leemos las líneas siguientes a la cabecera donde el server ennumera a los jugadores
                for j in range(i + 1, min(i + 1 + cantidad, len(lineas))):
                    linea_jugador = lineas[j].strip()

                    # El formato del log de PZ para jugadores suele ser "- NombreUsuario" o " - NombreUsuario"
                    match_nombre = re.search(r"-\s*([^\r\n]+)", linea_jugador)

                    if match_nombre:
                        nombre_real = match_nombre.group(1).strip()
                        jugadores.append(nombre_real)
                    elif linea_jugador:
                        # Si no tiene guión pero hay texto, tomar la línea completa limpia
                        jugadores.append(linea_jugador)

                return jugadores

    except Exception as error:
        print(f"❌ Error obteniendo jugadores: {error}")
